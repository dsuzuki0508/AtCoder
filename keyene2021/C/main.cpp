...
int main(void){
  ...
  //  DPRINT(m);
  cout<<-1<<endl;
  vector<vector<ModInt<998244353>>> v(H, vector<ModInt<998244353>>(W,0));
  v[0][0] = 3;
  v[0][0] = v[0][0].pow(H*W-K);
  for(int i = 0;i<H;i++){
    //  cout<<i<<endl;
    for(int j = 0;j<W;j++){
      if(m[i][j]=='R'){
	if(j<W-1)v[i][j+1]+=v[i][j];
      }else if(m[i][j]=='D'){
	if(i<H-1)v[i+1][j]+=v[i][j];
      }else if(m[i][j] == 'X'){
	if(j<W-1)v[i][j+1]+=v[i][j];
	if(i<H-1)v[i+1][j]+=v[i][j];
      }else{
	ModInt<998244353> tmp = v[i][j];
	tmp*=2;
	tmp/=3;
	if(j<W-1)v[i][j+1]+= tmp;
	if(i<H-1)v[i+1][j]+= tmp;
      }
    }
  }
  //  for(const auto& e:v)DPRINT(e);cout<<endl;
  cout<<v[H-1][W-1].val()<<endl;
