...
int main(void){
  ll H,W,K,V;
  cin>>H>>W>>K>>V;
  vector<vector<ll>> accum(H+1, vector<ll>(W+1, 0));
  for(ll i = 1;i<=H;i++){
    for(ll j = 1;j<=W;j++){
      cin>>accum[i][j];
      accum[i][j] = accum[i-1][j]+accum[i][j-1]-accum[i-1][j-1]+accum[i][j];
    }
  }
  //  for(auto e:accum)DPRINT(e);

  ll ans = 0;
  for(ll sizex = 1;sizex<=H;sizex++){
    for(ll sizey = 1;sizey<=W;sizey++){
      for(ll i = 1; i+sizex-1<=H ;i++){
	for(ll j = 1; j+sizey-1<=W ;j++){
	  if(accum[i+sizex-1][j+sizey-1] - accum[i-1][j+sizey-1] - accum[i+sizex-1][j-1]+ accum[i-1][j-1] + sizex*sizey*K> V)continue;
	  ans = max(ans, sizex*sizey);
	}
      }
    }
  }
