...
bool f(vector<set<tuple<ll,ll>>>& edge, ll X){
  ...
  using elm = tuple<ll,ll,ll,ll>;
  priority_queue<elm, std::vector<elm>, greater<elm>> que;
  que.push({0,0,INF,INF});
  while(!que.empty()){
    if(
