...
int main(void){
  ...
  for(int i = 0;i<N;i++){
    ...
    it = prev(it);
    /*if(it-x.begin() == i)next[i].push_back(INF);
      else */
    next[i].push_back(it-x.begin());
  }
  for(int k=0;pow(2,k)<N;k++){
    for(int i = 0;i<N;i++){
      if(next[i][k]==INF)next[i].push_back(INF);
      else next[i].push_back( next[next[i][k]][k] );
    }
  }
  for(const auto& e:next)DPRINT(e);
