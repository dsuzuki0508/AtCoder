...
int main(void){
  ...
  set<ll> a,b;
  for(ll i = 1;i*i<=ab[0].first;i++){
    if(ab[0].first%i==0){
      a.insert(i);
      a.insert(ab[0].first/i);
    }
  }
  for(ll i = 1;i*i<=ab[0].second;i++){
    if(ab[0].second%i==0){
      b.insert(i);
      b.insert(ab[0].second/i);
    }
  }
  ...
  auto solve = [&](ll a, ll b){
    for(const auto& e:ab){
      if( (e.first%a == 0) && (e.second%b == 0) )continue;
      if( (e.first%b == 0) && (e.second%a == 0) )continue;
      return false;
    }
    return true;
  };
