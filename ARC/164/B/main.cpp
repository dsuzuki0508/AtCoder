...
int main(void){
  ...
  for(int i = 0;i< N;i++){
    ...
    set<int> tmp;
    while(!que.empty()){
      auto [node, c, prev_color] = que.front();
      que.pop();
      if(prev_color==color[node]){
	if(checked.count(node))continue;
	if(cost[node]<0)continue;
	ok=true;
	break;
      }else{
	if(cost[node]>=0)continue;
	...
	for(auto e:edge[node]){
	  que.push({e, c+1, color[node]});
	}
      }
    }
    for(auto e:tmp)checked.insert(e);
    if(ok)break;
  }
