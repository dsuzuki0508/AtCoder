...
int main(void){
  ...
  auto f = [&](bool b, int n){
    static map<pair<bool,int>, int> conv;
    static int cnt = 0;
    if(conv.contains({b,n}))return conv[{b,n}];
    else return conv[{b,n}] = cnt++;
  };
  ...
  for(const auto& [X,Y,Z]:v){
	...
	s.insert(tmp);
	s.insert(m-tmp);
	ff.add_edge(f(false, 0), f(false, tmp), 1);
	ff.add_edge(f(false, tmp), f(true, m-tmp), 1);
	ff.add_edge(f(true, m-tmp), f(true, 0), 1);
      }
    };
    g(X, X*Y*Z);
    g(Y, X*Y*Z);
  ...
  int c = s.size();
  cout<<2*c - ff.max_flow(f(false, 0), f(true, 0))<<endl;
