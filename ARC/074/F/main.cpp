...
class FordFulkerson{
  vector<map<int,int>> mEdge;
  void resize(int n);
public:
  FordFulkerson(){}
  FordFulkerson(int n):FordFulkerson(){resize(n);}
  void add_edge(int from, int to, int cap);
  int max_flow(int s, int t);
};
void FordFulkerson::resize(int n){
  if(mEdge.size() < n )mEdge.resize(n);
}
void FordFulkerson::add_edge(int from, int to, int cap){
  resize(max(from, to)+1);
  mEdge[from][to]=cap;
}
int FordFulkerson::max_flow(int s, int t){
  resize(max(s,t)+1);
  vector<map<int,int>> edge = mEdge;
  vector<bool> visited(edge.size(), false);
  const int INF = numeric_limits<int>::max();

  auto dfs = [&](auto dfs, int node, int flow) -> int{
    if(node == t){
      return flow;
    }
    visited[node]=true;
...
int main(void){
  ...
  for(int i = 0;i<H;i++){
	...
	ff.add_edge(N, i, INF);
	ff.add_edge(N, j+H, INF);
	//	for(int k = 0;k<H;k++){
	//	  if(a[k][j]=='o')ff.add_edge(N, k, 1);
	//	}
      }else if(a[i][j]=='T'){
	ff.add_edge(i, N+1, INF);
	ff.add_edge(j+H, N+1, INF);
	//	for(int k = 0;k<W;k++){
	//	  if(a[i][k]=='o')ff.add_edge(k, N+1, 1);
	//	}
      }
    }
  }
  int ans = ff.max_flow(N, N+1);
