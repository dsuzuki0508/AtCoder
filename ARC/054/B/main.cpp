...
double solve(double n, double P){
  return n + P/pow(2, n/1.5);
}
...
int main(void){
  ...
  while(right-left > 1e-9){
    double tmp1 = left+(right-left)*1/3;
    double tmp2 = left+(right-left)*2/3;
    if(solve(tmp1,P)<solve(tmp2,P)){
      right=tmp2;
    }else{
      left=tmp1;
    }
  }
  double wait = (left+right)/2;
  cout<<fixed<<setprecision(9)<<solve(wait,P)<<endl;
