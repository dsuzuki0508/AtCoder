...
unsigned long long int combination(unsigned long long int n, unsigned long long int r){
  static vector<vector<unsigned long long int>> memo(2000000, vector<unsigned long long int>(20,INF));
  if(memo[n][r]!=INF)return memo[n][r];
  ModInt<998244353> ret;

  ret = factorial(n, 998244353);
  ret /= factorial(r, 998244353);
  ret /= factorial(n-r, 998244353);
  return memo[n][r] = ret.val();
}
...
int main(void){
  ...
  for(int i = 1;i<=M;i++){
    ...
    for(const auto& e:p){
      auto [prime, count] = e;
      cnt*=combination(N+count-1, count);
    }
    ans+=cnt;
  }
  cout<<ans.val()<<endl;
