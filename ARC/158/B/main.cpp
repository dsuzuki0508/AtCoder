  ...
  */
  vector<double> x;
  {
    ...
    for(int i = 0;i<N;i++){
      int tmp;
      cin>>tmp;
      de.push_back(1.0/tmp);
    }
    sort(de.begin(), de.end());
    for(int i = 0;i<3;i++){
      if(de.size()>0){
	x.push_back(1.0/de.front());
	de.pop_front();
      }
      if(de.size()>0){
	x.push_back(1.0/de.back());
	de.pop_back();
      }
    }
    sort(x.begin(), x.end());
  ...
  for(int i = 0;i<x.size();i++){
	...
	l=min(l, (double)(x[i] + x[j] + x[k]) / x[i] / x[j] / x[k]);
	u=max(u, (double)(x[i] + x[j] + x[k]) / x[i] / x[j] / x[k]);
      }
    }
  }
  cout<<fixed<<setprecision(14)<<l<<endl;
  cout<<fixed<<setprecision(14)<<u<<endl;
