...
bool f(vector<tddd>& v, double m){
  for(int i = N;i<N+M;i++)get<2>(v[i])=m;

  for(int i = 0;i<v.size();i++){
    for(int j = i+1;j<v.size();j++){
      auto [x1, y1, r1] = v[i];
      auto [x2, y2, r2] = v[j];
      if(pow(x1-x2, 2) + pow(y1-y2, 2) < pow(r1+r2, 2))return true;
    }
  }
  return false;
}
int main(void){
  ...
  cout<<fixed<<setprecision(15)<<upr<<endl;
