...
int main(void){
  cin>>N>>M;
  vector<vector<int>> accum(M, vector<int>(N+1,0));
  cnt.assign(M,0);
  for(int i = 0;i<N;i++){
    for(int j=0;j<M;j++)accum[j][i+1]=accum[j][i];
    int m;
    cin>>m;m--;
    accum[m][i+1]++;
    cnt[m]++;
  }
  vector<int> dp(1<<M, 0x3fffffff);
  dp[0]=0;
  for(int s = 1;s<(1<<M);s++){
    for(int i = 0;i<M;i++){
      if((s&(1<<i))==0)continue;
      ...
      for(int j=0;j<M;j++){
	if(i==j)continue;
	tmp += accum[j][l+cnt[i]] - accum[j][l];
      }
      dp[s] = min(dp[s], dp[ss]+tmp);
    }
  }
