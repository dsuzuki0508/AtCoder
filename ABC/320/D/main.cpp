...
int main(void){
  ll N,M;
  cin>>N>>M;
  vector<vector<pair<ll,ll>>> edge(N,vector<pair<ll,ll>>(N, {INF,INF}));
  for(ll i = 0;i<N;i++)edge[i][i] = make_pair(0,0);
  //  DPRINT(edge);
  for(ll i = 0;i<M;i++){
    ll a,b,x,y;
    cin>>a>>b>>x>>y;
    a--;b--;
    edge[a][b]=make_pair(x,y);
    edge[b][a]=make_pair(0,0)-make_pair(x,y);
  }
    DPRINT(edge);
  ...
  que.push({0,make_pair(0,0)});
  while(!que.empty()){
    auto [node,point] = que.front();
    que.pop();
    if(ans[node].first!=INF)continue;
    ans[node]=point;
    for(ll i = 0;i<N;i++){
      if(edge[node][i].first==INF)continue;
      que.push({i,point+edge[node][i]});
    }
  }
    DPRINT(ans);
  for(auto e:ans){
