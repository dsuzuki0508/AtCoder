...
int main(void){
  ll N,A,B,C;
  cin>>N>>A>>B>>C;
  vector<vector<vector<ll>>> edge(2, vector<vector<ll>>(N, vector<ll>(N,0x7fffffffffffffff)));
  for(ll i = 0;i<N;i++){
    for(ll j=0;j<N;j++){
      ll D;
      cin>>D;
  ...
  using pll = tuple<ll,ll,ll>;
  priority_queue<pll, vector<pll>, greater<pll>> que;
  que.push({0,0,0});
  vector<vector<ll>> ans(2, vector<ll>(N, -1));
  while(!que.empty()){
    auto [cost, densha,node] = que.top();que.pop();
    if(ans[densha][node]>=0)continue;
    ans[densha][node]=cost;
    for(ll i =0;i<N;i++){
      que.push({cost+edge[densha][node][i], densha, i});
    }
    if(densha==0)que.push({cost, 1,node });
  }
  cout<<min(ans[0].back(), ans[1].back())<<endl;
