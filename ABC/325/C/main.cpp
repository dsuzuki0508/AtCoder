...
int main(void){
  ...
  for(int i = 0;i<H;i++){
    for(int j = 0;j<W;j++){
      if(S[i][j]!='#')continue;
      if(i>0&&j>0&&'#'==S[i-1][j-1])uf.unite({i,j},{i-1,j-1});
      if(i>0&&'#'==S[i-1][j])uf.unite({i,j},{i-1,j});
      if(i>0&&j<W-1&&'#'==S[i-1][j+1])uf.unite({i,j},{i-1,j+1});
      if(j>0&&'#'==S[i][j-1])uf.unite({i,j},{i,j-1});
    }
  }
  set<int> cnt;
  for(int i = 0;i<H;i++){
