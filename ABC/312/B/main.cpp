...
bool check(int y, int x, vector<string>& S){
  for(int i=y;i<y+3;i++){
    for(int j=x;j<x+3;j++){
  ...
  for(int i=y+6;i<y+9;i++){
    for(int j=x+6;j<x+9;j++){
      if(S[i][j]=='.')return false;
    }
  }
  for(int i = y;i<y+4;i++)if(S[i][x+3]=='#')return false;
  for(int j = x;j<x+4;j++)if(S[y+3][j]=='#')return false;
  for(int i = y+5;i<y+9;i++)if(S[i][x+5]=='#')return false;
  for(int j = x+5;j<x+9;j++)if(S[y+5][j]=='#')return false;
  return true;
}
