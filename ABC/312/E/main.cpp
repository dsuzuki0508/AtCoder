...
int main(void){
  int N;
  cin>>N;
  vector<vector<vector<int>>> id(SIZE+1, vector<vector<int>>(SIZE+1, vector<int>(SIZE+1, -1)));
  for(int i = 0;i<N;i++){
    int x,y,z,X,Y,Z;
    cin>>x>>y>>z>>X>>Y>>Z;
    for(int j=x;j<X;j++){
  ...
  for(int i = 0;i<SIZE;i++){
	...
	f(id[i][j][k], id[i+1][j][k]);
	f(id[i][j][k], id[i][j+1][k]);
	f(id[i][j][k], id[i][j][k+1]);
      }
    }
  }
  for(auto& e:ans)cout<<e.size()<<endl;
