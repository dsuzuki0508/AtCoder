...
int main(void){
  ...
  auto dfs = [&](auto dfs, int n, int from) -> void{
    if(next[n].size())return;
    next[n].push_back(from);
    for(const auto& e:edge[n])dfs(dfs, e, n);
  };
  dfs(dfs, 0, -1);
  ...
  for(int k = 0;pow(2,k)<N;k++){
    for(int i = 0;i<N;i++){
      if(next[i][k]==-1)next[i].push_back(-1);
  ...
  depth[0]=0;
  auto d = [&](int n){
    auto rd = [&](auto rd, int n)->int{
      if(depth[n]!=INF)return depth[n];
      return depth[n] = rd(rd,next[n][0])+1;
    };
    return rd(rd, n);
  };

  auto anc = [&](int a, int n){
    for(int k = 20;k>=0;k--){
      if((n>>k)&1)a=next[a][k];
    }
    return a;
  ...
  auto lca = [&](int a, int b){
    if(d(a)<d(b))swap(a,b);
    if(d(a)>d(b))a = anc(a, d(a)-d(b));
    int l = 0,r=d(a);
    while(l!=r){
      int m = (l+r)/2;
      if(anc(a,m)==anc(b,m))r=m;
      else l = m+1;
    }
    return anc(a,l);
  };
