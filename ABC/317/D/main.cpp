...
int main(void){
  ...
  for(ll i=0;i<N;i++){
    ...
    cin>>x>>y>>z;
    sum+=z;
    obj.push_back({max(0ll, (y-x+1)/2), z});
  }
  DPRINT(obj);
  //  sum=(sum+1)/2;
  vector<vector<ll>> dp(N+1, vector<ll>(sum+1, 0x3fffffffffffff));

  dp[0][0]=0;
  //  DPRINT(dp[0]);
  for(ll i=0;i<N;i++){
    auto [hito, giseki] = obj[i];
    for(ll j = 0;j<= sum;j++){
      dp[i+1][j]=dp[i][j];
      dp[i+1][j]=min(dp[i+1][j], dp[i][max(0ll,j-giseki)]+hito);
    }
    //    DPRINT(dp[i+1]);
  }
  cout<<dp[N][(sum+1)/2]<<endl;
