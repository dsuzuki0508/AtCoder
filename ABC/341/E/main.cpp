...
using S = tuple<bool, bool, int>;
S op(S& a, S& b){
  DPRINT(make_pair(a,b));
  if(get<2>(a)==0)return b;
  if(get<2>(b)==0)return a;
  if(!get<1>(a) || !get<1>(b)){
    return {get<0>(a), false, get<2>(a)+get<2>(b)};
  }
  if(get<0>(b)){
    return {get<0>(a), true,  get<2>(a)+get<2>(b)};
  }
  return {get<0>(a), false, get<2>(a)+get<2>(b)};
}
...
int main(void){
  ...
  string str;
  cin>>str;
  using segtree = SegmentTree<S,op,e>;
  segtree seg(N+1);
  char prev = '0';
  for(int i = 0;i<N;i++){
    char c = str[i];
    if(c==prev){
      seg.set(i, {false, true, 1});
    }else{
      seg.set(i, {true, true, 1});
    }
    prev=c;
  }
  ...
  while(Q--){
    ...
    if(query==1){
      {
	auto [a,b,c] = seg.get(L);
	seg.set(L, {!a, b, c});
      }
      {
	auto [a,b,c] = seg.get(R+1);
	seg.set(R+1, {!a, b, c});
      }
    }else{
      auto [a,b,c] = seg.prod(L, R+1);
      DPRINT( seg.prod(L, R+1) );
      if(b)cout<<"Yes"<<endl;
      else cout<<"No"<<endl;
    }
  }
