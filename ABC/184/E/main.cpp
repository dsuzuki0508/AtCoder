...
int main(void){
  ...
  for(int i = 0;i<H;i++){
    for(int j = 0;j<W;j++){
      ...
      if('a'<=a[i][j]&& a[i][j]<='z')m[a[i][j]].push_back({i,j});
      //      if(a[i][j]!='#')a[i][j]='.';
    }
  }
  ...
  que.push({0,S});
  while(!que.empty()){
    auto [cost, p] = que.front();
    que.pop();
    if(p.first <0 || H<=p.first  ||
       p.second<0 || W<=p.second )continue;
    if(a[p.first][p.second]=='#')continue;
    if(ans[p.first][p.second] !=INF)continue;
    ans[p.first][p.second]=cost;
    if(p==G)break;

    if(!(p.first+1 <0 || H<=p.first+1  ||
	 p.second<0   || W<=p.second   ||
	 ans[p.first+1][p.second] != INF ||
	 a[p.first+1][p.second]=='#'))que.push({cost+1, {p.first+1, p.second}});
    if(!(p.first-1 <0 || H<=p.first-1  ||
	 p.second<0   || W<=p.second   ||
	 ans[p.first-1][p.second] != INF ||
	 a[p.first-1][p.second]=='#'))que.push({cost+1, {p.first-1, p.second}});
    if(!(p.first <0   || H<=p.first    ||
	 p.second+1<0 || W<=p.second+1 ||
	 ans[p.first][p.second+1] != INF ||
	 a[p.first][p.second+1]=='#'))que.push({cost+1, {p.first, p.second+1}});
    if(!(p.first <0   || H<=p.first    ||
	 p.second-1<0 || W<=p.second-1 ||
	 ans[p.first][p.second-1] != INF ||
	 a[p.first][p.second-1]=='#'))que.push({cost+1, {p.first, p.second-1}});
    ...
    if('a'<=c&& c<='z' && m.contains(c)){
      for(const auto& e:m[c]){
	if(ans[e.first][e.second] !=INF)continue;
	que.push({cost+1, e});
      }
      m.erase(c);
    }
  }
  //  for(const auto& e:ans)  DPRINT(e);
  if(ans[G.first][G.second]==INF) cout<<-1<<endl;
  else cout<<ans[G.first][G.second]<<endl;
  return 0;
}
