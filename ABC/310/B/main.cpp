...
bool c1(const set<int>& fi, const set<int>& fj){
  for(const auto& e:fi){
    if(!fj.count(e))return false;
  }
  return true;
}

bool c2(int i, int j, set<int> fi, set<int> fj){
  if(i>j)return true;
  for(const auto& e:fi){
    fj.erase(e);
  }
  if(fj.size())return true;
  return false;
}
...
int main(void){
  ...
  for(int i = 0;i< N;i++){
    for(int j = 0;j< N;j++){
      if(i==j)continue;
      if(P[i]>=P[j] && c1(F[i], F[j]) && c2(P[i], P[j], F[i], F[j] ) ){
	ans=true;
      }
    }
    if(ans)break;
