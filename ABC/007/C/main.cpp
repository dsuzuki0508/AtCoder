...
int main(void){
  ...
  cin>>R>>C;
  pair<int,int> s,g;
  cin>>s.first>>s.second;s.first--;s.second--;
  cin>>g.first>>g.second;g.first--;g.second--;
  ...
  que.push({s,0});
  while(!que.empty()){
    pair<pair<int,int>,int> e = que.front();
    que.pop();
    pair<int,int> p = e.first;
    int cost = e.second;
    if(map[p.first][p.second]=='#')continue;
    if(res[p.first][p.second]!=-1)continue;
    res[p.first][p.second]=cost;
    vector<pair<int,int>> d={{-1,0},{1,0},{0,-1},{0,1}};
    for(int i = 0;i< 4;i++){
      pair<int,int> tmp = {p.first+d[i].first, p.second + d[i].second};
      que.push({tmp, cost+1});
    }
  }
  //  DPRINT(res);
  cout<<res[g.first][g.second]<<endl;
