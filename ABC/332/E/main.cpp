...
int main(void){
  ...
  vector<double> W(N);
  cin>>W;
  const double INF = 1e18*1e18;;
  double ans = INF;
  double avg = accumulate(W.begin(), W.end(), 0.0) /D;

  auto dfs = [&](auto dfs, int n, vector<double>& v) ->double {
    if(n>=N){
      double ret = 0;
      if(v.size()!=D)return INF;
    ...
    if(N-n+v.size()<D)return INF;

    double ret = INF;
    if(v.size()<D){
      v.push_back(W[n]);
      ret =min(ret, dfs(dfs, n+1, v));
      v.pop_back();
    }
  ...
  vector<double> tmpv;
  cout<<fixed<<setprecision(8)<<dfs(dfs, 0, tmpv)<<endl;
