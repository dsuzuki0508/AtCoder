...
int main(void){
  ...
  auto solve = [&](){
    ...
    for(int i = 0;i<H;i++){
      int tmp = find(HI.begin(), HI.end(), i)-HI.begin();
      if(tmp>=HI.size()){
	return;
      }
      v[i] = A[tmp];
    ...
    */
    vector<int> WI(W);
    for(int i = 0;i<W;i++)WI[i]=i;
    do{
      vector<vector<int>> w(H, vector<int>(W,0));
      for(int i = 0;i<W;i++){
	int tmp = find(WI.begin(), WI.end(), i)-WI.begin();
	if(tmp>=WI.size()){
	  return;
	}
	for(int j = 0;j<H;j++){
	...
	for(int i = 0;i<WI.size();i++){
	  for(int j = i+1;j<WI.size();j++){
	    if(WI[i]>WI[j])cnt++;
	  }
	}
	ans = min(ans, cnt);
      }
    }while(next_permutation(WI.begin(), WI.end()));
  };

  do{
    solve();
  }while(next_permutation(HI.begin(), HI.end()));
