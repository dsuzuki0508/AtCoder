...
int main(void){
  ...
  ABC.push_back(B);
  ABC.push_back(C);
  auto dfs = [&](auto dfs, ll x, ll y, vector<ll> v) -> bool {
    if(x<=0 || y<=0)return false;
    if(v.size()==1){
      if(x*y<v[0])return false;
      else return true;
    }
    ...
    for(int i =0;i<v.size();i++){
      ...
      ll tmpy = n/x;
      if(tmpy*x<n)tmpy++;
      if(dfs(dfs, x-tmpx, y, tmpv))return true;
      if(dfs(dfs, x, y-tmpy, tmpv))return true;
    }
    return false;
  };
  if(dfs(dfs, X, Y, ABC))cout<<"Yes"<<endl;
  else cout<<"No"<<endl;
