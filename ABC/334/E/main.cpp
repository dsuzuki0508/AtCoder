...
int main(void){
  ...
  UnionFind<pair<int,int>> uf;
  for(int i = 0;i<H;i++){
    for(int j = 0;j<W;j++){
      if(S[i][j]!='#')continue;
      if(i>0)if(S[i-1][j]=='#')uf.unite({i,j}, {i-1, j});
      if(j>0)if(S[i][j-1]=='#')uf.unite({i,j}, {i, j-1});
    }
  }
  set<int> s;
  for(int i = 0;i<H;i++){
    for(int j = 0;j<W;j++){
      if(S[i][j]!='#')continue;
      s.insert(uf.find({i,j}));
    }
  }
  ModInt<998244353> ans;
  int n=0;
  for(int i = 0;i<H;i++){
    for(int j = 0;j<W;j++){
      if(S[i][j]!='.')continue;
      set<int> tmp;
      if(i>0)if(S[i-1][j]=='#')tmp.insert(uf.find({i-1,j}));
      if(i<H-1)if(S[i+1][j]=='#')tmp.insert(uf.find({i+1,j}));
      if(j>0)if(S[i][j-1]=='#')tmp.insert(uf.find({i,j-1}));
      if(j<W-1)if(S[i][j+1]=='#')tmp.insert(uf.find({i,j+1}));
      ans+=(s.size()-tmp.size()+1);
      n++;
    }
  }
  ans/=n;
