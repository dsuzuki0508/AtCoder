...
int main(void){
  ...
  vector<vector<int>> row(H,vector<int>(26, 0)),col(W, vector<int>(26, 0));
  for(int i = 0;i<H;i++){
    for(int j=0;j<W;j++){
      char tmp;
      cin>>tmp;
  ...
  DPRINT(c);
  vector<bool> row_deleted(H,false),col_deleted(W, false);

  auto istarget = [&](vector<int> x){
    int total = 0;
    int k = 0;
  ...
  DPRINT(col);
    ...
    for(int i = 0;i<H;i++){
      ...
      if(istarget(row[i]))del_row.push_back(i);
    }
    for(int i = 0;i<W;i++){
      if(col_deleted[i])continue;
      if(istarget(col[i]))del_col.push_back(i);
    }
    for(int i :del_row){
      ch--;
      cont=true;
