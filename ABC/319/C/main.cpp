...
bool gakkari(int n){
  ...
  for(int i = 0;i<3;i++){
    if(i==col)continue;
    tmpi.push_back(c[row*3+i]);
    tmpb.push_back((visited >> (row*3+i)) & 1);
  }
  if(tmpi[0]==tmpi[1] && tmpb[0] && tmpb[1])return true;

  tmpi.clear(), tmpb.clear();
  for(int i = 0;i<3;i++){
    if(i==row)continue;
    tmpi.push_back(c[i*3+col]);
    tmpb.push_back((visited >> (i*3+col)) & 1);
  }
  if(tmpi[0]==tmpi[1] && tmpb[0] && tmpb[1])return true;
  ...
  if(row==col){
    for(int i = 0;i<3;i++){
      if(i==row)continue;
      tmpi.push_back(c[i*3+i]);
      tmpb.push_back((visited >> (i*3+i)) & 1);
    }
    if(tmpi[0]==tmpi[1] && tmpb[0] && tmpb[1])return true;
  }
  ...
  if(row+col==2){
    for(int i = 0;i<3;i++){
      if(i==row)continue;
      tmpi.push_back(c[i*3+2-i]);
      tmpb.push_back((visited >> (i*3+2-i)) & 1);
    }
    if(tmpi[0]==tmpi[1] && tmpb[0] && tmpb[1])return true;
  }
...
int f(){
  if(visited == 0x1ff)return 1;
  int ret = 0;
  for(int i = 0;i<9;i++){
    if((1<<i)&visited)continue;
    if(gakkari(i))continue;
    visited |= (1<<i);
    ret += f();
    visited ^= (1<<i);
  }
...
int main(void){
  ...
  //  visited = 0x1fe;
  //  DPRINT(gakkari(0));
  cout<<fixed<<setprecision(10)<<(double)f()/factorial(9)<<endl;
