...
Piece slide(const Piece& a, int n){
  Piece ret(4,"....");
  for(int i = 0;i<4;i++){
    if(0<=i+n && i+n<4)ret[i+n]=a[n];
  }
  return ret;
}
bool h(Piece a, Piece b, Piece c){
  for(int i = 0;i<4;i++){
    for(int j=0;j<4;j++){
...
bool f(Piece a, Piece b, Piece c){
  cout<<__func__<<endl;
  for(int i= -3;i<4;i++){
  for(int j= -3;j<4;j++){
  for(int k= -3;k<4;k++){
  ...
  for(int n= -3;n<4;n++){
    ...
    auto tmpb = g(b, k,l);
    auto tmpc = g(c, m,n);
    if(h(tmpa, tmpb,tmpc))return true;
  }
  }
  }
  }
...
int main(void){
  ...
  vector<string> t(4);
  vector<vector<Piece>> v(3);
  for(int i = 0;i<3;i++){
    cin>>t;
    v[i].push_back(t);
    for(int j = 0;j<3;j++){
      v[i].push_back(rot(v[i].back()));
    }
  }
  //  DPRINT(v);
