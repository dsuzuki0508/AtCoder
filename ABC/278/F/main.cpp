...
int main(void){
  ...
  for(int i = 0;i<N;i++){
    string str;
    cin>>str;
    S[i]=make_pair(str.front()-'a', str.back()-'a');
  }
  //  DPRINT(S);
  ...
  auto dfs = [&](auto dfs, int rem, int last) ->bool {
    if(dp[rem][last] != -1)return dp[rem][last] == 1;
    bool ret = false;
    for(int i = 0;i<N;i++){
      if( !( (1<<i)&rem ) )continue;
      if(S[i].first != last) continue;
      int tmp = rem ^ (1<<i);
      if( !dfs(dfs, tmp, S[i].second) ) ret = true;
    }
    if(ret) dp[rem][last]=1;
    else dp[rem][last]=0;
    return ret;
  };
