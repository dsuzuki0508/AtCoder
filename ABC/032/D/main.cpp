...
vector<pair<ll,ll>> f(){
  ...
  for(int i = 1;i<ret.size();i++){
    ret[i].second = max(ret[i].second, ret[i-1].second);
...
void solve1(){
  ...
  for(int i = 0;i<(1<<v.size());i++){
    ...
    if(tmpw>W)continue;
    ll tmp = lower_bound(b.begin(), b.end(), make_pair(W-tmpw,-1ll))- b.begin();
    if(tmp>=b.size())tmp=b.size()-1;
    if(b[tmp].first > W-tmpw && tmp>0)tmp--;
    //    DPRINT(tmp);
    tmpv+= b[tmp].second;
    ans = max(ans, tmpv);
  }
...
void solve2(){
  vector<ll> dp(min(W, 200*1000)+1, 0);
  for(int i = 0;i<N;i++){
    //    DPRINT(dp);
...
void solve3(){
  vector<ll> dp(200*1000+1, 0x3fffffffffffffff);
  dp[0]=0;
  for(int i = 0;i<N;i++){
    for(int j = dp.size()-1;j>=0;j--){
      if(j-v[i]>=0)dp[j] = min(dp[j], dp[j-v[i]]+w[i]);
      if(j+1<dp.size())dp[j] = min(dp[j], dp[j+1]);
    }
  }
  /*
  {
    auto tmp = dp;
    tmp.resize(lower_bound(tmp.begin(), tmp.end(), 0x3fffffffffffffff)-tmp.begin()+1);
    DPRINT(tmp);
    }*/
  //  DPRINT(dp);
  ll i = upper_bound(dp.begin(), dp.end(), W)-dp.begin();
  i--;
  cout<<i<<endl;
}
...
int main(void){
  ...
  else if((*max_element(v.begin(), v.end())) <= 1000) solve3();
