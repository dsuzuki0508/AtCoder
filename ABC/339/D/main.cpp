...
int main(void){
  ...
  vector<string> m(N);
  cin>>m;
  m.insert(m.begin(), string(N, '#'));
  m.insert(m.end(), string(N, '#'));
  for(auto& s:m){
    s= "#"+s+"#";
  }
  //  for(auto& s:m)DPRINT(s);

  //  using Player = pair<int,int>;
  //  using Players = tuple<Player, Player>;
  using Players = ll;
  Players stat;
  ...
  //  map<Players, int> costs;
  unordered_set<Players> visited;
  queue<pair<Players, int>> que;
  que.push({stat, 0});
  int ans = -1;
  //  DPRINT(bitset<24>(stat));
  while(!que.empty()){
    auto [stat, cost] = que.front();
    que.pop();
    if(visited.contains(stat))continue;
    visited.insert(stat);
    if( (stat&0xfff)==((stat>>12)&0xfff) ){
      ans=cost;
      break;
    }
    ...
    for(const auto& d:dir){
      ...
      tmp>>=6;
      y1 = tmp&0x3f;
      //      DPRINT(make_tuple( y1, x1, y2, x2));

      {
	pair<int,int> p = make_pair(y1, x1)+d;
	if(m[p.first][p.second]=='.'){
      ...
      {
	pair<int,int> p = make_pair(y2, x2)+d;
	if(m[p.first][p.second]=='.'){
	  y2 = p.first;
	  x2 = p.second;
	}
