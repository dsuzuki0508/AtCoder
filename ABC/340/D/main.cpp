...
int main(void){
  ...
  for(ll i = 0;i<N-1;i++){
    ...
    edge[i][i+1] = A;
    edge[i][X] = B;
    if(X==i+1)edge[i][X] = min(A,B);
  }
  for(const auto& e:edge)  DPRINT(e);
  ...
  while(!que.empty()){
    ...
    DPRINT(make_pair(node, cost));
    
    //    if(visited.contains(node) && visited[node]< cost){
    //      continue;
    //    }
    //    visited[node]=cost;
    ...
    for(const auto& [to, time]:edge[node]){
      que.push({cost+time, to});
    }
  }
  cout<<ans<<endl;
