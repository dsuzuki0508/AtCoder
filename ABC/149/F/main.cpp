...
template<auto MOD>
std::ostream& operator<<(std::ostream& os, const ModInt<MOD>& n) {
  os << n.val();
  return os;
}


vector<map<int,ModInt<1000000007>>> edge;
  int N;


void f(int from, int node){
  if(edge[node][from]!=0){
    edge[from][node] = N-edge[node][from];
    return;
  }
  ...
  for(auto& e:edge[node]){
    ...
    if(e.second==0)f(node, e.first);
    result += e.second;
  }
  edge[from][node]=result;
  //  return result;
}
...
int main(void){
  ...
  for(int i = 0;i<N;i++){
	...
	f(i, e.first);
      ...
      ModInt<1000000007> tmp = 2;
      tmp = tmp.pow(e.second.val());
      tmp -= 1;
      cnt -= tmp;
      //      cnt += 1;
      //  DPRINT(e.second.val());
