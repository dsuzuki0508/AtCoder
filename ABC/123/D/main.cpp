...
int main(void){
  ...
  priority_queue<tllll> que;
  que.push({A[X-1]+B[Y-1]+C[Z-1], X-1, Y-1, Z-1});

  using tlll=tuple<ll,ll,ll>;
  set<tlll> used;
  for(int ii = 0;ii<K;){
    auto [sum, i, j, k] = que.top();
    que.pop();
    if(used.count({i,j,k}))continue;
    used.insert({i,j,k});
    cout<<sum<<endl;
    ii++;
    if(i>0)que.push({A[i-1]+B[j]+C[k], i-1, j, k});
    if(j>0)que.push({A[i]+B[j-1]+C[k], i, j-1, k});
    if(k>0)que.push({A[i]+B[j]+C[k-1], i, j, k-1});
  }
