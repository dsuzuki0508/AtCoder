...
int N,M;
vector<vector<bool>> stopped;
vector<vector<bool>> touched;
vector<string> m;
void solve(int y, int x){
  //  DPRINT(y);
  //  DPRINT(x);
  if(stopped[y][x])return;
  stopped[y][x] = true;
  touched[y][x]=true;
  int i = 0;
  for( i = y;m[i][x]=='.';i--)touched[i][x]=true;
  solve(i+1,x);
  for( i = y;m[i][x]=='.';i++)touched[i][x]=true;
  solve(i-1,x);
  for( i = x;m[y][i]=='.';i--)touched[y][i]=true;
  solve(y,i+1);
  for( i = x;m[y][i]=='.';i++)touched[y][i]=true;
  solve(y,i-1);
}
...
int main(void){
  cin>>N>>M;
  stopped.assign(N,vector<bool>(M));
  touched.assign(N,vector<bool>(M));
  m.resize(N);
  cin>>m;
  solve(1,1);
  int ans = 0;
