...
int main(void){
  ...
  vector<pair<int,int>> query(Q), orig;
  for(auto& e:query){
    cin>>e;
    e.first--;
  }
  ...
  auto it1 = query.begin();
  auto it2 = query.begin();
  for(int i = 0;i*SIZE<N;i++){
    //      DPRINT(query);
    it1 = lower_bound(it2, query.end(), make_pair(i*SIZE,0));
    it2 = lower_bound(it1, query.end(), make_pair((i+1)*SIZE,0));
    //    DPRINT(it1-query.begin());
    //    DPRINT(it2-query.begin());
    
    if(i&1){
      sort(it1, it2, [](pair<int,int> a, pair<int,int> b){
