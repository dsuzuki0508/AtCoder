...
bool f(string s, string t){
  if(s==t)return true;
  if(s.size()+1==t.size() && g(s,t))return true;
  if(t.size()+1==s.size() && g(t,s))return true;
...
int main(void){
  ...
  for(int i = 0;i<T.size()+1;i++){
    for(char c = 'a';c<='z';c++){
      s.insert(T.substr(0,i)+c+T.substr(i) );
    }
  }
  for(int i = 0;i<T.size();i++){
    //    DPRINT(T.substr(0,i)+T.substr(i+1));
  ...
  for(int i = 0;i<N;i++){
    string S;
    cin>>S;
    if(s.count(S)) ans.push_back(i+1);
  }
