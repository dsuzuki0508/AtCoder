...
int main(void){
  ...
  for(int i = 0;i<N-1;i++){
    que.push({f(v[vx[i].second], v[vx[i+1].second]), vx[i].second, vx[i+1].second});
  }
  for(int i = 0;i<N-1;i++){
    que.push({f(v[vy[i].second], v[vy[i+1].second]), vy[i].second, vy[i+1].second});
  }
  DPRINT(que);
  UnionFind<int> uf;
  int ans = 0;
