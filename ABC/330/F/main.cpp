...
int main(void){
  ...
  DPRINT(X);
  DPRINT(Y);
  int ans = max(abs(X.back()-X.front()), abs(Y.back() -Y.front()));
  DPRINT(ans);
  auto compress = [&](vector<int> src, vector<int>& decomp){
    

    swap(src, decomp);
    decomp.erase(unique(decomp.begin(), decomp.end()), decomp.end());
  };
  vector<int> decompx, decompy;
  compress(X, decompx);
  compress(Y, decompy);
  ...
  auto f = [&](vector<int> decomp, map<int,int> cnt){
    int cntl = cnt[decomp.front()];
    int cntr = cnt[decomp.back()];
    int l = 0;
    int r = decomp.size()-1;
    int size = decomp[r]-decomp[l];
    map<int,pair<ll, int>> ret;
    ll cost=0;
    ret[size]= make_pair(cost, min(cntl,cntr));
    //    DPRINT(cnt);
    //    DPRINT(decomp);
    while(l<r){
      //      DPRINT(cntl);
      ...
      //      DPRINT(cost);
      if(cntl<cntr){
	cost += cntl*(decomp[l+1] - decomp[l]);
	cntl+=cnt[decomp[l+1]];
	l++;
      }else{
	cost += cntr*(decomp[r] - decomp[r-1]);
	cntr+=cnt[decomp[r-1]];
	r--;
      }

      size = decomp[r]-decomp[l];
      ret[size]=make_pair(cost, min(cntl, cntr));
    }
    //	DPRINT(ret);
    return ret;
  ...
  //  DPRINT(f(decompy, cnty));
  map<int, pair<ll, int>> mpx, mpy;
  mpx= f(decompx, cntx);
  mpy= f(decompy, cnty);


  auto g = [&](ll n){
    int x = upper_bound(mp{n,0})
  };
