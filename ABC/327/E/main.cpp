...
double f(int n){
  ...
  if(n<=0)return 0.00000000001;
  if(n<ans.size())return ans[n];
  else ans.resize(n+1);
  return ans[n] = f(n-1)*0.9 +1;
}
int main(void){
  int N;
  cin>>N;
  vector<double> P(N);
  cin>>P;
  vector<vector<double>> dp(N, vector<double>(N+1, numeric_limits<double>::lowest()));
  dp[0][1] = P[0];

  for(int i = 1;i<P.size();i++){
    dp[i][1]=P[i];
    for(int j = 0;j<=i+1;j++){
      if(j>0)dp[i][j]=max({dp[i][j], dp[i-1][j], dp[i-1][j-1]*0.9 +P[i]});
    }
  }
  //  for(const auto& e:dp)DPRINT(e);
  ...
  for(int i = 1;i<dp.back().size();i++){
    ...
    DPRINT(dp.back()[i]);
    DPRINT(f(i));
    DPRINT(dp.back()[i]/f(i));
    DPRINT(1200/pow((double)i, 0.5));
    DPRINT(dp.back()[i] / f(i) -  1200/pow((double)i, 0.5));
    double tmp = dp.back()[i] / f(i) -  1200/pow((double)i, 0.5);
    DPRINT(tmp);
    */
    ans = max(ans, dp.back()[i] / f(i) -  1200/pow((double)i, 0.5));
  }
