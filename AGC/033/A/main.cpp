...
int main(void){
  ...
  int ans = 0;
  while(!que.empty()){
    auto [y,x,cost] = que.front();que.pop();
    if(m[y][x]!=inf)continue;
    m[y][x] = cost;
    ans = max(ans, cost);
    if(y>0)  que.push({y-1, x, cost+1});
    if(y<H-1)que.push({y+1, x, cost+1});
    if(x>0)  que.push({y, x-1, cost+1});
    if(x<W-1)que.push({y, x+1, cost+1});
  }
  for(const auto& e:m)DPRINT(e);cout<<endl;
