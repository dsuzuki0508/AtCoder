...
ll solve(  ll N, ll K, vector<ll> A){
...

  {
    ...
    tmp.push_back(x);
    while(s.size()>0){
      auto it = s.upper_bound(tmp.back());
      if(it==s.end()){
	it = s.begin();
      }
      tmp.push_back(*it);
  ...
  for(ll i = 0;i<N-1;i++){
    ans+=(A[i]-A[i+1]+K)%K;
    //    DPRINT((A[i]-A[i+1]+K)%K);
    m = min(m, (A[i]-A[i+1]+K)%K);
    //    DPRINT(ans);
  }
  ans += (A[N-1]-A[0]+K)%K;
  //  DPRINT((A[N-1]-A[0]+K)%K);
  m = min(m, (A[N-1]-A[0]+K)%K);
  // cout<<ans-m<<endl;
  return ans-m;
}

ll verify(  ll N, ll K, vector<ll> A){
  sort(A.begin(), A.end());
  ll ans = 0;
  do{
    ll sum=0;
