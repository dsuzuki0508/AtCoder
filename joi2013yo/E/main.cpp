...
int main(void){
  int N,K;
  cin>>N>>K;
  vector<ll> comx, comy,comd;
  map<ll,ll> decx, decy,decd;
  using tllllll = tuple<ll,ll,ll,ll,ll,ll>;
  vector<tllllll> o;
  for(int i=0;i<N;i++){
    int x1,y1,d1,x2,y2,d2;
    cin>>x1>>y1>>d1>>x2>>y2>>d2;
    o.push_back({x1,y1,d1,x2,y2,d2});
    comx.push_back(x1);
    comx.push_back(x2);
    comy.push_back(y1);
    comy.push_back(y2);
  ...
  DPRINT(comy);
  DPRINT(comd);
  auto f=[&](vector<ll>& c, map<ll,ll>& d){
    sort(c.begin(), c.end());
  ...
  vector<vector<vector<ll>>> accum(comx.size()+1,vector<vector<ll>>(comy.size()+1, vector<ll>(comd.size()+1, 0)));

  for(const auto& e:o){
    auto [x1,y1,d1,x2,y2,d2] = e;
    for(int i = decx[x1];i<decx[x2];i++){
      for(int j = decy[y1];j<decy[y2];j++){
	for(int k = decd[d1];k<decd[d2];k++){
	  accum[i][j][k]++;
	}
      }
    }
  ...
  for(int i = 0;i<accum.size();i++){
    for(int j = 0;j<accum[i].size();j++){
      for(int k=0;k<accum[i][j].size();k++){
	if(accum[i][j][k]<K)continue;
	ans += (comx[i+1]-comx[i])*(comy[j+1]-comy[j])*(comd[k+1]-comd[k]);
      }
    }
  }
  cout<<ans<<endl;
