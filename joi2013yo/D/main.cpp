...
std::istream& operator>>(std::istream& is, std::tuple<Args...>& t){
  SCAN_TUPLE_WITH_SEQUENCE(is, t, std::make_index_sequence<std::tuple_size<std::tuple<Args...>>::value>{});
  return is;
}
...
int main(void){
  ...
  for(int i = 1;i<D;i++){
    for(int j  =0;j<N;j++){
      if(T[i]<get<0>(C[j]) || get<1>(C[j])< T[i])continue;
      dp[i][j] = 0;
      for(int k=0;k<N;k++){
	if(T[i-1]<get<0>(C[k]) || get<1>(C[k])< T[i-1])continue;
	dp[i][j] = max(dp[i][j], dp[i-1][k] + abs(get<2>(C[k]) - get<2>(C[j])));
      }
      if(i==D-1)ans=max(ans,dp[i][j]);
    }
    //    DPRINT(dp[i]);
  }
  cout<<ans<<endl;
