...
void g(vector<vector<int>>& v, int yy, int xx){
...
int main(void){
  ...
  int n;
  cin>>n;
  using tllll = tuple<int,int,int,int>;
  vector<tllll> area;
  vector<int> decx, decy;
  decx.push_back(0);
  decx.push_back(w);
  decy.push_back(0);
  decy.push_back(h);
  map<int,int> comx,comy;
  for(int i=0;i<n;i++){
    int x1, y1, x2, y2;
    cin>>x1>>y1>>x2>>y2;
    area.push_back({x1, y1, x2, y2});
    decx.push_back(x1);
    decx.push_back(x2);
    decy.push_back(y1);
    decy.push_back(y2);
  }
  auto f=[&](vector<int>& d, map<int,int>& c){
    sort(d.begin(),d.end());
  ...
  vector<vector<int>> m(decy.size(), vector<int>(decx.size(), 0));
