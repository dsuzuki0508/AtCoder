...
template <typename T>
std::ostream& operator<<(std::ostream& os, BinaryIndexedTree<T>& c) {
  vector<T> v;
  for(int i = 0;i<c.size();i++){
    v.push_back(c.sum(i, i+1));
  }
  os<<v;
  return os;
}
...
int main(void){
  ...
  {
    vector<int> tmp;
    for(const auto& [a,b]: v)tmp.push_back(a);
  ...
  BinaryIndexedTree<ll> s(N+1); //放出量
  BinaryIndexedTree<ll> t(N+1); //人数
  ...
  for(int i = 0;i<N;i++){
    ...
    while(r-l>1){
      int m = (l+r)/2;
      if( t.sum(0, m)*b < s.sum(0, m) ) r=m;
      else l = m;
    }
    DPRINT(l);
    DPRINT(t.sum(0,l));
    ans = max(ans, t.sum(0, l));
  }
  cout<<ans<<endl;
