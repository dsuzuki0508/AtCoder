...
std::tuple<T,U,V> operator+(const std::tuple<T,U,V>& l, const std::tuple<T,U,V>& r){
...
int main(void){
  ...
  int K;
  cin>>M>>N>>K;
  vector<vector<tlll>> accum(1,vector<tlll>(N+1, {0,0,0}));
  for(int i = 0;i<M;i++){
    accum.push_back(vector<tlll>(1, {0,0,0}));
    for(int j=0;j<N;j++){
      char c ; cin>>c;
      tlll tmp;
      if(c=='J')tmp = {1,0,0};
      else if(c=='O')tmp = {0,1,0};
      else tmp = {0,0,1};
      accum[i+1].push_back(accum[i+1].back() + accum[i][j+1] - accum[i][j] + tmp);
    }
  }
  //  DPRINT(accum);
  for(int i = 0;i<K;i++){
    int a,b,c,d;
    cin>>a>>b>>c>>d;
    tlll ans = accum[c][d] + accum[a-1][b-1] - accum[a-1][d] - accum[c][b-1];
