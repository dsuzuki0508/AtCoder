...
template<typename T>
bool isBipartite(const T& graph){
  auto s = *(*graph.begin()).begin();
  map<decltype(s), bool> color;

  auto dfs = [&](auto dfs, decltype(s) node, bool c)-> bool{
    if(color.contains(node)){
      return color[node] == c;
    }
    color[node]=c;
    for(const auto& e: graph.at(node)){
      if(!dfs(dfs, e, !c))return false;
    }
    return true;
  };
  for(const auto& e: graph){
    if( !color.contains(*e.begin()) && !dfs(dfs, *e.begin(), false)) return false;
  }
  return true;
}
...
int main(void){
  ...
  map<ll,bool> col;
  auto dfs = [&](auto dfs, ll n, bool c){
    if(col.contains(n))return;
    col[n]=c;
    c=!c;
    for(const auto& e:edge[n]){
      dfs(dfs, e, c);
    }
  };
  dfs(dfs, 0, false);
